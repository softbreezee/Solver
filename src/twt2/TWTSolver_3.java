...
public class TWTSolver_3 {
	...
	public TWTSolver_3() throws Exception{
		//初始化
...
//		g.fileOutput();
	...
	public static void main(String[] args) throws Exception {
		TWTSolver_3 sovler = new TWTSolver_3();
		int[] arr = {3,2};
		sovler.truckNum = arr;
		sovler.tMax =8;
...
//				cplex.addEq(oneNodeVist, oneNodeVisted); 
				...
				for (int i = stockNum; i < stockNum + taskNum; i++) {
					for (int j = 0; j < stockNum + taskNum; j++) {
						if (i != j 	) {
							...
							con3.addTerm(1.0, Y[i - stockNum][k]);
							con3.addTerm(-1.0, Y[j - stockNum][k]);
			...
			IloIntVar[][][] U = new IloIntVar[stockNum+taskNum][taskNum][stockNum+taskNum];
			for(int i = 0;i<stockNum+taskNum;i++){
				for(int j = 0;j<taskNum;j++){
					for(int k = 0;k<stockNum+taskNum;k++){
						U[i][j][k] = cplex.boolVar();
					}
				}
			}
			...
			for(int k = 0;k<trucks;k++){
				for (int i = 0; i < taskNum+stockNum; i++) {
					for (int j = 0; j < taskNum+stockNum; j++) {
						for (int m = stockNum; m < taskNum+stockNum; m++) {
					cplex.addLe(cplex.prod(nodeTW[0][m-stockNum], U[i][m-stockNum][j]), Y[m-stockNum][k]);
					cplex.addGe(cplex.prod(nodeTW[1][m-stockNum], U[i][m-stockNum][j]), Y[i-stockNum][k]);
						}
					}
				}
			}
				...
				for (int i = stockNum; i < taskNum + stockNum; i++) {
					for (int j = 0; j <stockNum; j++) {
						if (j != i) {
							...
							drivePersistence.addTerm(1.0, Y[i-stockNum][k]);
						}
					}
				}
				for (int i = 0; i < stockNum; i++) {
					for (int j = stockNum; j <stockNum+taskNum; j++) {
						if (j != i) {
							drivePersistence.addTerm(-1.0, Y[j-stockNum][k]);
							drivePersistence.addTerm(transTime[i][j], X[i][j][k]);
						}
					}
				}
				...
				for (int i = stockNum; i < taskNum + stockNum; i++) {
					for (int j = 0; j <stockNum; j++) {
						if (j != i) {
							...
							exprObj.addTerm(1.0, Y[i-stockNum][k]);
						}
					}
				}
			}
			for(int k = 0;k<trucks;k++){
				for (int i = 0; i < stockNum; i++) {
					for (int j = stockNum; j <stockNum+taskNum; j++) {
						if (j != i) {
							exprObj.addTerm(-1.0, Y[j-stockNum][k]);
							exprObj.addTerm(transTime[i][j], X[i][j][k]);
						}
					}
				}
